...
def get_mount_port():
    # Auto-detect the mount's serial port
...
def connect_to_mount():
    # Setup the mount with the desired slewing and tracking speeds
    ...
    if slewingSpeed != b'9#':
        logger.debug("Incorrect slewing rate, attempting to set it to 1440x sidereal")
        mountSerialPort.write(b':MSR9#')
        if mountSerialPort.read(1) == b'1':
            logger.debug("Succesfully updated slewing rate.")
        else:
